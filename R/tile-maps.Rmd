...
```{r}
theme_set(theme_void(base_size = 16, base_family = "Noto Serif")) #Editorial New
...
```{r map-bundesländer}
set.seed(1)
pal <- sample(viridis::turbo(n = 16))

ggplot(grid, aes(col, row, color = land, fill = land)) +
  geom_tile() +
  coord_fixed() +
  scale_x_continuous(limits = c(-.5, max(grid$col) + .5)) +
  scale_y_reverse() +
  scale_color_manual(values = pal, name = NULL) +
  scale_fill_manual(values = pal, name = NULL) + 
  theme(plot.margin = margin(15, 10, 15, 0)) +
  labs(caption = "Grafik: Cédric Scherer & Ansgar Wolsing")

ggsave(here::here("plots", "grid_länder.pdf"), width = 10, height = 9, device = cairo_pdf)
```
...
ggsave(here::here("plots", "bubble_map_diff_t.pdf"), width = 10, height = 13, device = cairo_pdf)
```
